...
int __registration_icp_ndt::init() {
    ...
    ndt->setMaximumIterations(80);                  //      设置匹配迭代的最大次数
    ndt->setTransformationEpsilon(0.05);            // 0.05 为终止条件设置最小转换差异
    ndt->setStepSize(0.007);                        //      为More-Thuente线搜索设置最大步长
    ndt->setResolution(0.07);                       //      设置NDT网格结构的分辨率（VoxelGridCovariance）（体素格的大小）
